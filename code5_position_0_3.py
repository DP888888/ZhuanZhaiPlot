...
# 可调参数
price = [200.0, 180.0, 170.0, 160.0, 155.0, 150.0]
expected_margin = 0.2
positions = []
...
def get_position_helper(data, end, index, r):  # 递归搜索所有的加仓可能性
    if index == r:
        ...
        for j in range(index):  # 计算各个价格反弹的时候收益率（按全部资金记）
            ...
            cost = 0.0
            revenue = 0.0
            SellPrice = ((price[0] - price[j - 1]) * 0.3 + price[j - 1])
            for k in range(j+1):
                total += data[k] * 10000/price[k]
                cost += data[k] * 10000
                # if j - 1 >= 0:
                #     revenue += data[k] * SellPrice
                # else:
                #     revenue += data[k] * price[0]
            if j == 0:
                revenue = total * price [0]
            else:
                revenue = total * SellPrice
            if cost == 0.0:
                results.append(0.0)
            else:
    ...
    for k in range(index):
        ...
        cost += data[k] * price[k]
        if index - 2 >= 0:
            revenue += data[k] * ((price[0] - price[index - 2]) * 0.3 + price[index - 2])
        else:
            revenue += data[k] * price[0]
